...
test('a saved query can be duplicated, and the copy is named for the original', async ({
  ...
  await typeQuery(page, 'select 1');
  await page.keyboard.press('ControlOrMeta+s');
  await page.getByRole('textbox', { name: /^Name$/ }).fill('Albums');
  await page.getByRole('dialog').getByRole('button', { name: 'Save', exact: true }).click();

  // The star in the rail is where saved queries live.
  await page.getByRole('button', { name: /Saved queries/i }).click();
  await expect(page.getByRole('button', { name: 'Albums', exact: true })).toBeVisible();

  await page
    .getByRole('button', { name: /Duplicate this query/i })
    .first()
    .click();
  await expect(page.getByRole('button', { name: 'Albums copy', exact: true })).toBeVisible();

  // And again, which numbers from two rather than stacking the word.
  await page
    .getByRole('button', { name: /Duplicate this query/i })
    .first()
    .click();
