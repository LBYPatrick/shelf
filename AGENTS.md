...
Guidance for working in this repository. `CLAUDE.md` imports this file.

## Repository skills

- [publish-release](.agents/skills/publish-release/SKILL.md): prepare release
  notes and publish a Shelf release. Use when asked to cut, ship, or publish a
  release. `.claude/skills/publish-release` links to the same skill.
...
- **Provider differences are declared, not thrown** — the same rule the database
  drivers follow. API adapters return tool calls to our agent loop; Claude Code,
  Codex and Grok Build use the per-turn loopback MCP bridge. All share the same
  read-only executor. Keep the capability catalogue aligned with the adapters.
